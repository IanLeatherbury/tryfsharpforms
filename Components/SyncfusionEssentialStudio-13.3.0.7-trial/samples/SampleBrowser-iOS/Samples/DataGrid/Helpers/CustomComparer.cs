...
namespace SampleBrowser
{
    public class CustomerInfo : IComparer<Object>, ISortDirection
	{
		public int Compare(object x, object y)
		{
			...
			//For Group type Data                                   
			else if (x.GetType() == typeof(Syncfusion.Linq.Group))
			{
				//Calculating the group key length
                namX = ((Syncfusion.Linq.Group)x).Key.ToString().Length;
                namY = ((Syncfusion.Linq.Group)y).Key.ToString().Length;
			}
			else
			{
    ...
    public class CustomSortComparer : IComparer<object>, ISortDirection
    {
        ...
        public int Compare(object x, object y)
        {
            ...
            {
                namX = ConverKeyToDouble((string)((Syncfusion.Linq.Group)x).Key);
                namY = ConverKeyToDouble((string)((Syncfusion.Linq.Group)y).Key);
            }
            else
            {
                namX = (double)x;
