	...
	public class Styles:SampleView
	{
		...
		void SegmentControl_ValueChanged (object sender, EventArgs e)
		{
			...
			if (value == 0) {
				...
				this.SfGrid.AlternatingRowColor = UIColor.FromRGB (230, 239, 237);
			} else if (value == 2) {
				this.SfGrid.GridStyle = new Red ();
				this.SfGrid.AlternatingRowColor = UIColor.FromRGB (252, 242, 240);
			} else if (value == 3) {
				this.SfGrid.GridStyle = new Green ();
				this.SfGrid.AlternatingRowColor = UIColor.FromRGB (248, 249, 229);
			}
		}

		void SfGrid_GridViewCreated (object sender, GridViewCreatedEvent e)
		{
			this.SfGrid.SelectedItem = viewmodel.OrdersInfo [1];
			this.SfGrid.GridStyle = new Dark();
		}
